;(function(win, doc, undefined) {
    ...
    function Snowflakes(_pageContainer,_snowflakesContainer) {
    	...
    	this.pageContainer	= doc.getElementById(_pageContainer);
    	this.snowflakesContainer = doc.getElementById(_snowflakesContainer);
    }
        ...
        for (var i = ss.length - 1; i >= 0; i--) {
            try {
                ...
                for (var j = rs.length - 1; j >= 0; j--) {
                    if ((rs[j].type === win.CSSRule.WEBKIT_KEYFRAMES_RULE || rs[j].type === win.CSSRule.MOZ_KEYFRAMES_RULE) && rs[j].name == a){
                        return rs[j];
                    }
                }
            }
    ...
    Snowflakes.prototype.create = Snowflakes.prototype.moreSnow = function(snowflakeCount) {
    	...
    	while (i < snowflakeCount) {
    		var snowflake	= doc.createElement('i');
